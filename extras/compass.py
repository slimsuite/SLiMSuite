    ...
    def _cmdList(self):     ### Sets Attributes from commandline
        ...
        for cmd in self.cmd_list:
            try:
                ...
                #    self.list['Server'] = string.split(rje.matchExp('^serverlist=(\S+)',cmd)[0],sep=',')
            except:
                ...
                try:
                    ...
                    for line in reslines:
                        ...
                        if scan:
                            ...
                            if aa == '.':
                                aa = sequence[4]
                            NP.write('%s\n' % string.join([accnum, pos, sequence, score, aa],'\t'))
                            _writex += 1
                    NP.close()
...
#########################################################################################################################
    def _compResults(self):  ### Populates objects as necessary and executes methods
        ...
        try:
            ...
            for seq in seqlist:
                TDT.write(string.join(['','Pos','AA','Score','Perc','SA'],'\t'))
                        ...
                        if seq in res_seq:  # Has residue
                            if res_scan.has_key(accnum) and res_scan[accnum].has_key(enz_info):  # Has this enzyme!
                                ...
                                if rsa['aa'] != seq.info['Sequence'][r]:
                                    ...
                                    #else:
                                    TDT.write(string.join(['','%d' % seqpos[seq],'*%s*' % rsa['aa'],rsa['score'],rsa['perc'],rsa['sa']],'\t'))
                                #print ['','%d' % seqpos[seq],res_scan[accnum]['aa'],rsa['score'],rsa['perc'],res_scan[accnum]['sa']]
                                else:
                                    TDT.write(string.join(['','%d' % seqpos[seq],rsa['aa'],rsa['score'],rsa['perc'],rsa['sa']],'\t'))
                            elif self.dict['Feature'].has_key(accnum):   # Residue but no enzyme
                                TDT.write('\t\t%s\t\t\t' % seq.info['Sequence'][r])
                            else:   # No scan
                                TDT.write(string.join(['','NULL',seq.info['Sequence'][r],'NULL','NULL','NULL'],'\t'))
                        else:   # No residue
                            TDT.write('\t-' * 5)
                ## <e> ## Checks
                _stage = '<1e> Checks'
