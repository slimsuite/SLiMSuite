        ...
        for i in range(len(self.cmd_list)):
            ...
            try:
                ...
                if cmd.find('hometitle="') == 0:
                    ...
                    self.info['HomeTitle'] = string.join(hometitle,' ')[:-1]
            except:
    ...
    def run(self):      ### Main run method
        ...
        try:
            ...
            if self.stat['Interactive'] >= 0 and not rje.yesNo('Proceed with these settings?'):
                self.edit()
            extlist = string.split(self.info['ExtList'],',')
            ## <b> ## Types, Groups and Elements
            folderpath = string.split(rje.makePath(self.info['PicFolder'],wholepath=True),os.sep)
            if len(folderpath) > 3:
            ...
            for type in typelist:
                groupdic[type] = rje.subDir(string.join([type,grouppath],os.sep),[self.info['ThumbNails']])
                elementdic[type] = {}
                pixdic[type] = {}
                for group in groupdic[type]:
                    if self.opt['ClearHTML']:
                        html_die = glob.glob(string.join([type,group,'*.htm'],os.sep)) + glob.glob(string.join([type,group,'*.html'],os.sep))
                        for file in html_die:
                            os.unlink(file)
                    elementdic[type][group] = rje.subDir(string.join([type,group,elpath],os.sep),[self.info['ThumbNails']])
                    pixdic[type][group] = {}
                    # Pictures
                    for el in elementdic[type][group]:
                        if self.opt['ClearHTML']:
                            html_die = glob.glob(string.join([type,group,el,'*.htm'],os.sep)) + glob.glob(string.join([type,group,el,'*.html'],os.sep))
                            for file in html_die:
                                os.unlink(file)
                        pixdic[type][group][el] = []
                        for ext in extlist:
                            piclist = glob.glob(string.join([type,group,el,'*.%s' % ext],os.sep))
                            for pic in piclist:
                                pixdic[type][group][el].append(string.split(pic,os.sep)[-1])
            ...
            for type in rje.sortKeys(pixdic):
                ...
                for group in rje.sortKeys(pixdic[type]):
                    while 1:
                        tdir = string.join([type,group,self.info['ThumbNails']],os.sep)
                        renamex = 0
                        missing = []
                        if not rje.checkForFile(tdir):
                            ...
                            for el in pixdic[type][group]:
                                for pix in pixdic[type][group][el]:
                                    pic = string.join([tdir,pix],os.sep)
                                    ...
                                    if not rje.checkForFile(tpic):  # Missing!
                                        if tpic[-4:].lower() == '.gif':
                                            self.verbose(0,1,'\n!!! GIF thumbnails not supported. Make thumbnail for %s as %s !!!' % (string.join([type,group,el,pix],os.sep),tpic),2)
                                        else:
                                            missing.append(pix)
            ...
            for type in rje.sortKeys(pixdic):
                ...
                for group in rje.sortKeys(pixdic[type]):
                    ...
                    HTML = open(string.join([type,group,'index.htm'],os.sep),'w')
                    ...
                    ## Group margin.htm
                    HTML = open(string.join([type,group,'%s_margin.htm' % group],os.sep),'w')
                    ...
                    HTML = open(string.join([type,group,'%s_thumbnails.htm' % group],os.sep),'w')
                    ...
                    for el in rje.sortKeys(pixdic[type][group]):
                        ...
                        HTML = open(string.join([type,group,el,'index.htm'],os.sep),'w')
                        ...
                        ## Element margin.htm
                        HTML = open(string.join([type,group,el,'%s_margin.htm' % el],os.sep),'w')
                        HTML.write('<HTML><HEAD><TITLE>%s Margin</TITLE></HEAD>\n\n' % eldesc[type][group][el])
                        if self.info['Background'] != 'None':
                            HTML.write('<BODY BACKGROUND="../../../%s" WIDTH="100%%">\n\n' % self.info['Background'])
                        HTML.write('<FONT FACE="%s" SIZE=-1><CENTER>\n' % self.info['FontFace'])
                        ...
                        HTML = open(string.join([type,group,el,'%s_thumbnails.htm' % el],os.sep),'w')
                        HTML.write('<HTML><HEAD><TITLE>%s Thumbnails</TITLE></HEAD>\n\n' % eldesc[type][group][el])
                        if self.info['Background'] != 'None':
                            HTML.write('<BODY BACKGROUND="../../../%s" WIDTH="100%%">\n\n' % self.info['Background'])
                        ...
                        HTML = open(string.join([type,group,el,'%s_downloads.htm' % el],os.sep),'w')
                        ...
                        ## Element downloads.htm
                        HTML = open(string.join([type,group,el,'%s_full_images.htm' % el],os.sep),'w');
                        ...
                        for pic in pixdic[type][group][el]:
                            HTML = open(string.join([type,group,el,'%s.htm' % self._picID(pic)],os.sep),'w')
                            HTML.write('<HTML><HEAD><TITLE>%s</TITLE></HEAD>\n\n' % self._picName(pic))
                            if self.info['Background'] != 'None':
                                HTML.write('<BODY BACKGROUND="../../../%s" WIDTH="100%%">\n\n' % self.info['Background'])
                            HTML.write('<FONT FACE="%s" SIZE=-1><CENTER>\n' % self.info['FontFace'])
