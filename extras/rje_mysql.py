        ...
        try:
            ...
            if self.opt['Combine'] and not self.opt['Append'] and os.path.exists(self.info['Name']) and self.stat['Interactive'] >= 0:
                if rje.yesNo('Output file %s already exists. Overwrite?' % self.info['Name']):
                    ...
                    return
            filelist = rje.getFileList(callobj=self,filelist=string.split(self.info['FileList'],','),subfolders=self.opt['SubFolders'])
...
#########################################################################################################################
    def _processFile(self,file,delimit=None):    ### Processes given File
        ...
        try:
            ...
            self.verbose(0,3,'\nDelimit: "%s"\nFirst line: %s' % (string.replace(delimit,'\\','\\\\'),firstline),1)
            ...
            while self.stat['Interactive'] >= 0:     # Cycle Through Menu
                ...
                if choice == 'S': # Skip this file
                    ...
                    for f in range(len(firstline)):
                        desclist[f] = '%s %s' % (os.path.basename(file),headers[f])                            
                elif choice == 'D' and rje.yesNo('Change delimiter from "%s"?' % string.replace(delimit,'\\','\\\\')):
        ...
        try:### ~ [0] ~ Setup ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            ...
            while line:
                if rje.matchExp('CREATE TABLE `(\S+)`',line):
                    ...
                    CSV.write('%s\n' % string.join(chead,','))
                    self.printLog('#HEAD','%s: %s' % (table,string.join(chead,',')))
                    create = False
                elif line.find('INSERT INTO `%s`' % table) == 0:
                    dx = 0
                    for entry in string.split(line,'(')[1:]:
                        output = rje.readDelimit(string.replace(entry[:entry.find(')')],"'",'"'),',')
...
#########################################################################################################################
    def _buildText(self,delimit='\t'):   ### Returns the auto-generated build statement for making a MySQL table
        ...
        buildlist.append('    );')
        ### Load Data ###
        loadcmd = "LOAD DATA LOCAL INFILE '%s' INTO TABLE %s" % (string.replace(self.info['File'],'\\','\\\\'),self.info['Name'])
        if delimit != '\t':
            loadcmd += " FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '\"'"
        if self.fieldlist[0].opt['AutoIncrement']:
            loadcmd += ' (%s)' % string.join(fieldlist[1:],', ')
        loadcmd += ';'
        ...
        return string.join(buildlist,sep='\n')
#########################################################################################################################
### End of Table Class                                                                                                  #
#########################################################################################################################
