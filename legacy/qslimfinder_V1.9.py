...
import slimfinder, rje_slim, rje_slimcalc, rje_slimlist
...
def makeInfo():     ### Makes Info object
    ...
    (program, version, last_edit, copyyear) = ('QSLiMFinder', '1.9', 'July 2014', '2008')
    description = 'Query Short Linear Motif Finder'
...
#########################################################################################################################
    def run(self,batch=False):  ### Main QSLiMFinder Run Method
        ...
        try:###~PRECHECK~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~###
            ...
            self.errorLog('Error in QSLiMFinder.run()',printerror=True,quitchoice=False)
...
#########################################################################################################################
    def myPickle(self):  ### Returns pickle identifier, also used for Outputs "Build" column (self.info['Build'])
        ...
        if self.getStrLC('Build'):
            ...
            if self.getBool('AlphaHelix'): self.setInfo({'Build':'alpha-o%da%d' % (self.getInt('AmbOcc'),amb)})
            elif self.getInt('MinWild'): self.setInfo({'Build':'l%dw%d-%do%da%d' % (self.getInt('SlimLen'),self.getInt('MinWild'),self.getInt('MaxWild'),self.getInt('AmbOcc'),amb)})
            else: self.setInfo({'Build':'l%dw%do%da%d' % (self.getInt('SlimLen'),self.getInt('MaxWild'),self.getInt('AmbOcc'),amb)})
        ...
        try:
            ...
            if load:
                ...
                for var in ['Masking','DisMask','ConsMask','MaskM','QExact']:   # Opt
                    if var not in newme.opt: changes.append(self.errorLog('Warning: "%s" parameter not found in pickle' % var, printerror=False, nextline=False))
                ...
                if newme.list['Query'] != self.list['Query'] or newme.info['Focus'] != self.getStr('Focus'):
