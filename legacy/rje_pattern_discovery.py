		...
		try:
			...
			### InfoContent ###
				...
				for motif in mypresto.motif:
					...
					pattern = rje.replace(motif.info['Sequence'],'X','.')
					elements = rje.split(pattern,'-')
					pattern = ''
					for el in elements:
...
#########################################################################################################################
	def _makeLength(self):	### Makes length from pattern
		...
		self.stat['Wildcards'] = rje.count(self.info['Pattern'],'.')
