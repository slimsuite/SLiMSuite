...
#########################################################################################################################
    def run(self):  ### Main run method
        ...
        try:### ~ [1] Load Data ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            ...
            self.list['PlotFT'] = rje.split(rje.join(self.list['PlotFT']).upper())
            if self.info['OccFile'].lower() not in ['','none']:
                ...
                self.obj['SeqList'].autoFilter(['GoodSeq=%s' % rje.join(rje.sortKeys(self.dict['OccData']),',')])
            ### ~ [2] Calculate Stats ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            self.list['PlotStat'] = rje.split(rje.join(self.list['PlotStat']).lower())
    ...
    def output(self,seq,outfile,occdata=[]):   ### Output to file
        ...
        try:### ~ [1] ~ Basic Data ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            ...
            odata = ['Name\t%s' % (seq.shortName()),
                     ...
                     'Output\t%s' % (rje.join(rje.split(outfile,'.')[:-1],'.')),
                     'RE\t%s' % (rje.join(self.list['PlotRE'],',')),
                     'TrueELMs\tY',
                     'Description\t%s' % (seq.info['Description']),
                     '',]
            ### ~ [2] ~ PlotStats ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            for plot in rje.sortKeys(seq.dict['PlotStat']): odata.append('Plot\t%s\t%s' % (plot,rje.join(seq.dict['PlotStat'][plot],', ')))
            ...
            ### ~ [5] ~ Output ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            open(outfile,'w').write(rje.join(odata,'\n'))
