...
import rje_slim, rje_slimcalc, rje_slimlist, slimmaker
...
def makeInfo():     ### Makes Info object
    ...
    (program, version, last_edit, copyyear) = ('SLiMFinder', '4.9', 'July 2014', '2007')
    description = 'Short Linear Motif Finder'
    ...
    comments = ['Cite: Edwards, Davey & Shields (2007), PLoS ONE 2(10): e967. [PMID: 17912346]',
                ...
                'Please report bugs to R.Edwards@Soton.ac.uk']
...
#########################################################################################################################
    def _setAttributes(self):   ### Sets Attributes of Object
        ...
        if self.log.info['Name'] == 'QSLiMFinder':
            ...
            self.setStr({'ResDir': self.getInfo('BuildPath'),'ResFile':'qslimfinder.csv'})
        ### Other Attributes ###
    ...
    def _cmdList(self):     ### Sets Attributes from commandline
        ...
        for cmd in self.cmd_list:
            try:
                ...
                self._cmdRead(cmd,'opt','PreAmb','ambiguity')
                self._cmdRead(cmd,'stat','ProbCut','sigcut')
...
#########################################################################################################################
    def setupMinOcc(self):  ### Adjusts MinOcc and AmbOcc settings according to UPNum etc.
        ...
        if self.getInt('AmbOcc') <= 0: self.setInt({'AmbOcc':self.getInt('MinOcc')})
        for occ in ['AmbOcc','MinOcc']:
            if self.getInt(occ) < 1:
...
#########################################################################################################################
    def loadAADimerFreq(self):  ### Calculates/loads AA Dimer Frequencies
        ...
        try:### ~ [1] Setup ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            ...
            slimbuild.setStat({'MinOcc':1,'MinWild':0,'MaxWild':max(self.getInt('MaxWild'),4)})
    ...
    def OLDpickleMe(self,load=False):  ### Loads existing pickle, or saves pickle for later!
        ...
        try:
            ...
            if load:
                ...
                newme.stat['StartTime'] = self.getNum('StartTime')
            ...
            ### Save ###
            if 'Pickle' in self.opt and not self.getBool('Pickle'):
...
#########################################################################################################################
    def cloudOutput(self):  ### Generates motif cloud output
        ...
        try:
            ...
            extrabase = self.runBase()
            cloudfile = '%s.cloud.txt' % extrabase
            ## General Cloud Data ##
