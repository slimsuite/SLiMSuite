...
import rje, rje_seq, rje_sequence, rje_scoring, rje_slim, rje_slimcalc, rje_slimlist, rje_zen
import rje_slimcore_V1 as rje_slimcore
#########################################################################################################################
...
def makeInfo():     ### Makes Info object
    ...
    (program, version, last_edit, copyyear) = ('SLiMProb', '1.4', 'July 2014', '2007')
    description = 'Short Linear Motif Probability tool'
...
#########################################################################################################################
    def _setAttributes(self):   ### Sets Attributes of Object
        ...
        self.infolist = ['AAFreq','AltDis','BuildPath','CaseMask','CompMask','GablamDis','Input','ResDir','ResFile',
                         'RunID','Background']
        self.optlist = ['DisMask','Force','EFilter','LogMask','Masked','Masking','MaskM','MaskFreq','SmearFreq',
        ...
        self.coreDefaults()
        self.setInfo({'BuildPath':rje.makePath('SLiMProb/'),'CompMask':'None',
                      'ResDir':rje.makePath('SLiMProb/'),'ResFile':'slimprob.csv'})
        self.setStat({'SeqOcc':1,'MaxSeq':0,'MaxSize':1e5,'Extras':2})
    ...
    def _cmdList(self):     ### Sets Attributes from commandline
        ...
        for cmd in self.cmd_list:
            try:### General Options ### 
                ...
                self._cmdReadList(cmd,'opt',['DisMask','Force','EFilter','LogMask','Masked','Masking','MaskM',
                                             'MaskFreq','SmearFreq','TarGZ','Teiresias','OccUPC'])
                self._cmdReadList(cmd,'int',['MaxSeq','SaveSpace','HomCut','SeqOcc','MaxOcc','Extras'])
                self._cmdReadList(cmd,'stat',['MST','WallTime','MaxSize'])
                self._cmdRead(cmd,'stat','MaxSize','maxaa')
...
#########################################################################################################################
    def run(self,batch=False):  ### Main SLiMProb Run Method
        ...
        try:### ~ [1] Batch/SeqIn input ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            ...
            self.errorLog('Error in SLiMProb.run()',printerror=True,quitchoice=False)
...
#########################################################################################################################
    def OLDpickleMe(self,load=False):  ### Loads existing pickle, or saves pickle for later!
        ...
        try:### ~ [1] Setup ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            ...
            elif not load and not self.getBool('Pickle'): return None  # Do not save pickle
            mypickle = self.myPickle()  ## Pickle name ##
            #!# Replace this name
            
            ### ~ [2] Load existing pickle ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            if load:
                newme = None    ## New SLiMProb object, loaded from Pickle
                ## Check for file and load ##
                #!# Replace with self.unpickleMe(basefile,process=False) = generic object method
                for pfile in [self.getStr('Basefile'),'%s%s' % (self.getStr('BuildPath'),self.dataset())]:
                    ...
                    if os.path.exists('%s.%s.pickle' % (pfile,mypickle)):
                        self.printLog('#LOAD','Attempting to load SLiMProb pickle.',log=False)
                        newme = pickle.load(open('%s.%s.pickle' % (pfile,mypickle),'r'))
                        self.printLog('#LOAD','SLiMProb intermediate loaded: %s.%s.pickle.' % (pfile,mypickle))
                        if not self.getBool('Win32'):
                            try:
                                ...
                                self.printLog('#GZIP','SLiMProb %s.%s.pickle zipped.' % (pfile,mypickle))
                ...
                for var in ['CompMask','CaseMask','MotifMask']:         # Info
                    if self.getStr(var) != newme.info[var]: changes.append(self.errorLog('Warning: "%s" parameter mismatch' % var, printerror=False, nextline=False))
                for var in ['Masking','DisMask','MaskM','ConsMask']:   # Opt
                    if self.opt[var] != newme.opt[var]: changes.append(self.errorLog('Warning: "%s" parameter mismatch' % var, printerror=False, nextline=False))
                ...
                newme.setStr({'ResFile':self.getStr('ResFile'),'ResDir':self.getStr('ResDir'),
                              'BuildPath':self.getStr('BuildPath'),'Basefile':self.getStr('Basefile')})
                newme.setNum({'StartTime':self.getNum('StartTime')})
                newme.setInt({'Extras': self.getInt('Extras')})
            ...
            if 'Pickle' in self.opt and not self.opt['Pickle']:
                self.printLog('#PICKLE','SLiMProb pickling disabled with pickle=F.',log=True)
                return None
            self.printLog('#SAVE','Attempting to save SLiMProb with pickle.',log=False)
            pickle.dump(self,open('%s.%s.pickle' % (self.getStr('Basefile'),mypickle),'w'))
            self.printLog('#SAVE','SLiMProb intermediate saved as %s.%s.pickle (Python pickle).' % (self.getStr('Basefile'),mypickle))
    ...
    def processPickle(self,newme):  ### Changes attributes accordingly
        ...
        try:### ~ [1] ~ Setup ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            ...
            for var in ['CompMask','CaseMask','MotifMask']:         # Info
                if self.getStr(var) != newme.info[var]: changes.append(self.errorLog('Warning: "%s" parameter mismatch' % var, printerror=False, nextline=False))
            for var in ['Masking','DisMask','MaskM','ConsMask']:   # Opt
                if self.getBool(var) != newme.opt[var]: changes.append(self.errorLog('Warning: "%s" parameter mismatch' % var, printerror=False, nextline=False))
            for var in ['FTMask','IMask']:  #x#,'Equiv']:      # List
                slist = self.list[var][0:]
                nlist = newme.list[var][0:]
                slist.sort()
