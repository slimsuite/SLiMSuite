...
setTesting = function(){
  ...
  #arglist = list("rtype"="pacbio","basefile"="/Users/redwards/Documents/Projects/YeastPacBio-Jun15/analysis/2015-08-14.PAGSAT.MBG8150/MBG8150.SP16496.hcq.qv20.sgd.srt.PAGSAT/MBG8150.SP16496.hcq.qv20.sgd.srt.L1000ID99","refbase"="/Users/redwards/Documents/Projects/YeastPacBio-Jun15/data/2015-07-01.S288C.8150.Ref/sgd.srt")
  #arglist = list(rtype="pagsat",basefile="/Users/redwards/Data/projects/Microbiogen-Nov14/analysis/2015-08-12.PAGSAT/MBG11A.SP16492.hcq.qv20.sgd.srt.PAGSAT/MBG11A.SP16492.hcq.qv20.sgd.srt.L500ID80",refbase="/Users/redwards/Data/projects/YeastPacBio-Jun15/data/2015-07-01.S288C.8150.Ref/sgd.srt")
}
...
#># [1] "Summary"   "HitNum"    "Length"    "Coverage"  "Identity"  "Positives" "Missing"   "Errors"    "Perfect"   "N"
...
#># [1] "Chrom"     "Pos"       "HitNum"    "ContigNum" "Contigs"   "Class"     "ChromHit"  "ChromNum"  "RefChrom" 
...
#>#  [1] "Qry"       "Hit"       "AlnNum"    "BitScore"  "Expect"    "Length"    "Identity"  "Positives" "QryStart"  "QryEnd"    "SbjStart"  "SbjEnd"    "Dirn"      "QryCol"   
#># [15] "HitCol"   
...
chromGenesPlot = function(chromdata,genedata,protdata,chrom,ctype="Chromosome"){
  ...
  ymin = 0
  cname = strsplit(chrom,"_")[[1]][1]
  geneplot = genedata[genedata$Chrom==cname,]  # If plotting a contig, will want to change this to $Hit
  geneplot$x = (geneplot$Start+geneplot$End)/2
  protplot = protdata[protdata$Chrom==cname,]  # If plotting a contig, will want to change this to $Hit
  protplot$x = (protplot$Start+protplot$End)/2
  # Setup Plot
  plot(c(0,xmax),c(ymin,ymax),col="red",type="n",main=paste(settings$basename,"-",chrom,"-",xmax,settings$units),ylab="Top gene/protein hits (%identity)",xlab=paste(ctype,"Position",units),xaxs="i")
...
## ~ Local stack plots ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ##
localChromPlot = function(chromdata,chrom,hitlist,ctype="top"){
  # Plot of the Features on a Chromosome:
  scaling = settings$scaling
  units = paste("(",settings$units,")",sep="")
  plotdata = chromdata[chromdata$Chrom==chrom,] 
  ...
  ymax = length(hitlist)
  # Setup Plot
  plot(c(0,xmax),c(0,ymax),col="red",type="n",main=paste(chrom,ctype,"contigs hit stack"),ylab="",xlab=paste("Chromosome Position",units),xaxs="i",yaxt="n")
  plotGridlines(0,xmax,0,ymax,10000/scaling,50000/scaling,1,ymax)
  # Plot Features
  for(fi in 1:ymax){
    y = ymax-fi
...
summGraph = function(summdata){
  ...
  #sumtypes = c("Reference","Assembly","ReferenceAlign","AssemblyAlign","Self","Genes","Genes.Reciprocal","Proteins","Proteins.Reciprocal")
  #sumtext = c("Reference","Assembly","Ref.Align","AssemblyAlign","Ref.Self","Ref.Genes","Map.Genes","Ref.Proteins","Map.Proteins")
  sumtypes = c("Reference","Assembly","ReferenceAlign","AssemblyAlign","Self","Genes","Proteins")
  sumtext = c("Reference","Assembly","Ref.Align","AssemblyAlign","Ref.Self","Ref.Genes","Ref.Proteins")
  ymax = 7   # dim(summdata)[1] - output all even missing ones
  par(mar=c(5,6,2,1))
  ...
  for(fi in 1:ymax){
    ...
    if(dim(ydata)[1] > 0){
      ...
      rect(0,y+0.5,100.0*ydata$Identity/ydata$Coverage,y,col=soton$col[10])
...
coverageByChromosome = function(covdata,locdata,features,gtype="Reference"){
  ...
  for(fi in 1:ymax){
    ...
    if(gtype == "Reference"){
      ldata = locdata[locdata$Qry == as.character(chroms[fi]),]
      ...
      ldata = locdata[locdata$Hit == as.character(chroms[fi]),]
...
alignedChromosomes = function(covdata,alndata,features){
  ...
  for(fi in 1:ymax){
    ...
    # Plot mapped local hits
    ldata = alndata[alndata$Query == as.character(chroms[fi]),]
    ldata = ldata[order(ldata$Length),]
...
for(chrom in levels(chromdata$Chrom)){
  ...
  for(contig in levels(locdata$Hit)){
    ...
    # The problem with using hitlen-normalised data is that lots of small fragments can overwhelm the big players
    # >> maybe replace with a min (sum/hitlen) threshold ??? = settings$topcontigprop
    ssum = sum(sdata$Length)/hitlen
    if(ssum >= settings$topcontigprop){
      covsum = c(covsum,sum(sdata$Length))
    }else{
      covsum = c(covsum,ssum)  # Will rank those > settings$topcontigprop by total length, rest by %covered
    }
  }
  (covsum/10000)
  (levels(locdata$Hit))
  # Replace the hitlist with contigs that have unique coverage and/or top gene/protein hits?
  (hitlist = levels(locdata$Hit)[order(covsum,decreasing=TRUE)[1:settings$topcontigs]])
  localChromPlot(chromdata,chrom,hitlist,ctype=paste("top",settings$topcontigs))
  par(mar=c(5,6,2,1))
