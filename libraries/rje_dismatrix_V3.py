...
def todo():     ### Major Functionality to Add - only a method for PythonWin collapsing! ###
    ...
    # [ ] : Add reading of subset of a data file.
    '''
#########################################################################################################################
def makeInfo():     ### Makes Info object
    '''Makes rje.Info object for program.'''
    (program, version, last_edit, cyear) = ('RJE_DISMATRIX', '3.0', 'August 2014', '2007')
    description = 'Distance Matrix Module'
...
#########################################################################################################################
class DisMatrix(rje_obj.RJE_Object):
    '''
    Distance Matrix Class. Author: Rich Edwards (2007). This class can handle *positive* distances only!
...
#########################################################################################################################
    def _setAttributes(self):   ### Sets Attributes of Object
        ...
        ### Basics ###
        self.strlist = ['Name','Type','Description','OutMatrix']
        self.boollist = ['Symmetric','nsf2nwk']
        self.intlist = []
        self.numlist = []
        self.filelist = []
        self.listlist = []
    ...
    def _cmdList(self):     ### Sets Attributes from commandline
        ...
        for cmd in self.cmd_list:
            try:
                ...
                ### Class Options ###
                self._cmdRead(cmd,type='str',att='OutMatrix')
                self._cmdRead(cmd,type='file',att='Name',arg='loadmatrix')
                self._cmdReadList(cmd,'bool',['Symmetric','nsf2nwk'])
...
#########################################################################################################################
    def run(self):  ### Will try simple loading and saving of distance matrix
        ...
        try:### ~ [1] ~ Load distance matrix ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            ...
            try: filext = mat_ext[self.getStrLC('OutMatrix')]
            except: filext = 'csv'
...
#########################################################################################################################
    def checkSymmetry(self,force=False):    ### Checks symmetry of matrix and forces if desired
        ...
        try:### ~ [1] Assess symmetry ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            ...
            self.setBool({'Symmetric':sym})
            if force: self.forceSymmetry()
            return self.getBool('Symmetric')
...
#########################################################################################################################
    def forceSymmetry(self,method='min',missing=-1):   ### Compresses an Asymmetrical matrix into a symmetrical one
        ...
        '''
        ### Setup ###
        if self.getBool('Symmetric'): return
        self.setBool({'Symmetric':True})
        if method not in ['min','max','mean']:
            self.errorLog('Force Symmetry method "%s" not recognised. Will use "min".' % method,printerror=False)
            method = 'min'
        ### Force symmetry ###
...
#########################################################################################################################
    def loadFromDataTable(self,filename=None,delimit=None,clear=True,key1='Qry',key2='Hit',distance='Qry_OrderedAlnID',normalise=100.0,inverse=True,checksym=False,objlist=[],fkey='Load'):  ### Loads distance matrix from e.g. GABLAM output
        '''
        ...
        >> objlist:list [] = Whether to restrict loading to objlist. Must match single-word names at start of loaded table lines.
        >> fkey:str ['Load'] = Key for self.file if using objlist.
        '''
        try:### ~ [1] Setup filename and options. Check for file ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            if not filename: filename = self.getStr(fkey)
            if not filename: filename = self.name()
            if not os.path.exists(filename):
                self.errorLog('Matrix data file "%s" not found' % filename,printerror=False)
                return False
            ...
            ## ~ [2a] Load Dictionary ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ##
            if objlist:
                self.printLog('\r#DIS','Extracting %s objects from %s...' % (rje.iLen(objlist),filename))
                data = {};
                self.setStr({fkey:filename})
                FILE = self.open(fkey)
                headers = []
                while not headers:
                    headers = rje.readDelimit(FILE.readline(),delimit)
                    while headers and not headers[-1]: headers.pop(-1)
                try:
                    i1 = headers.index(key1)
                    i2 = headers.index(key2)
                    di = headers.index(distance)
            ...
            self.printLog('\r#DIS','Reading "%s" matrix from %s: %s distances read.' % (distance,filename,rje.integerString(dtot)))
            
            ### ~ [3] Check symmetry of matrix ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            if checksym: self.checkSymmetry(force=self.getBool('Symmetric'))
            return True
        except:
...
#########################################################################################################################
    def loadMatrix(self,filename=None,delimit=None,clear=True,usecase=False,checksym=True,default=0.0):   ### Loads distance matrix 
        ...
        try:### ~ [1] Setup filename and options. Check for file ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            if not filename: filename = self.name()
            if not os.path.exists(filename):
                self.errorLog('Matrix file "%s" not found' % filename,printerror=False)
                return False
            if not delimit: delimit = rje.delimitFromExt(filename=filename)
            if clear: self.dict['Matrix'] = {}
            ## ~ [1a] Check for BLOSUM ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ##
...
#########################################################################################################################
    def saveMatrix(self,objkeys=None,filename='matrix.txt',delimit=',',format=None,log=True,default=1.0):   ### Saves matrix 
        ...
        try:### ~ [1] Setup formatting ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            ...
            if not format: format = self.getStr('OutMatrix')
            if format == 'None' and self.getBool('MySQL'): format = 'mysql'
...
#########################################################################################################################
    def saveCytoscape(self,basename=None,type='dis',cutoff=1.0,inverse=False,selflink=False):   ### Outputs matrix in Cytoscape format
        ...
        '''
        try:### ~ [1] Setup ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            if not basename: basename = rje.baseFile(self.name())
            ...
            for obj1 in objlist:
                for obj2 in objlist:
                    if self.getBool('Symmetric') and objlist.index(obj2) < objlist.index(obj1): continue
            ...
            SIF.close()
            EDGE.close()
            self.printLog('#SIF','%s matrix output in Cytoscape format to %s and %s' % (basename,sif,eoa))
        except: self.errorLog('rje_dismatrix.saveCytoscape(%s) failure' % basename)
#########################################################################################################################
